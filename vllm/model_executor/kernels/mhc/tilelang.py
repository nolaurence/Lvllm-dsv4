...
from .torch import (
    ...
    mhc_post_torch,
    mhc_pre_broadcast_torch,
    mhc_pre_torch,
)


def _should_use_sm86_reference() -> bool:
    from vllm.utils.deep_gemm import _use_sm86_reference

    return _use_sm86_reference()
...
def _torch_hc_prenorm_gemm(
    ...
    assert out.shape[0] == sqrsum.shape[0]
    x_float = x.float()
...
def mhc_pre_broadcast_tilelang(
    ...
    if _should_use_sm86_reference():
        return mhc_pre_broadcast_torch(
            residual,
            fn,
            hc_scale,
            hc_base,
