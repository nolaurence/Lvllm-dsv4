...
def FusedMoEFactory(
    ...
    if routed_experts_cls is None:
        ...
        if should_use_lk_moe(layer_name):
            if enable_eplb:
                raise RuntimeError(
                    "EPLB is not compatible with LK-MoE NUMA mode. Disable "
                    "EPLB or set LVLLM_MOE_NUMA_ENABLED=0."
                )
            routed_experts_cls = LkRoutedExperts
        else:
