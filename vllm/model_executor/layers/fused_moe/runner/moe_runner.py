...
class MoERunner(MoERunnerInterface):
    ...
    def _apply_quant_method(
        ...
        if getattr(self.routed_experts, "lk_moe", None) is not None:
            # Start shared experts before the blocking CPU expert path so their
            # CUDA work can overlap routing and NUMA execution.
            self._maybe_apply_shared_experts(
                shared_experts_input,
            ...
            fused_out = self.routed_experts.forward_lk(  # type: ignore[attr-defined]
                hidden_states,
                topk_weights,
                topk_ids,
            )
