...
def _fp32_to_fp8_e4m3fn_byte(x):
    ...
    #   fp8_exp >= 1 → fp8 normal
    #   fp8_exp <= 0 → fp8 subnormal or zero after rounding
    is_zero_input = fp32_exp == 0
    is_normal = fp8_exp >= 1
    is_subnormal = (fp8_exp <= 0) & ~is_zero_input

    # Subnormal mantissa: mag * 2^9 rounded to nearest even. A rounded
    # mantissa of 8 is the smallest normal value (encoding 0x08).
    fp_value = abs_bits.to(tl.float32, bitcast=True)
    sub_mant_f = fp_value * 512.0  # 2^9
    sub_truncated = sub_mant_f.to(tl.int32)
    sub_remainder = sub_mant_f - sub_truncated.to(tl.float32)
    sub_round_up = (sub_remainder > 0.5) | (
        (sub_remainder == 0.5) & ((sub_truncated & 1) == 1)
    )
    sub_mant = sub_truncated + sub_round_up.to(tl.int32)
    sub_mant = tl.minimum(sub_mant, 8)
...
def w8a8_triton_block_scaled_mm(
    ...
    try:
        ...
        if _use_sm86_reference():
            return _w8a8_block_scaled_mm_pyref(A, B, As, Bs, block_size, output_dtype)
    except Exception:
        pass
...
try:
    ...
    def _stable_buf_pair(
        ...
        key = (
            tuple(data_ref.shape),
            tuple(data_ref.stride()),
            str(data_ref.dtype),
            tuple(scale_ref.shape),
            tuple(scale_ref.stride()),
            str(scale_ref.dtype),
