...
def _dequantize_and_gather_k_cache_pyref(
    ...
    gls = gather_lens.long() if gather_lens is not None else seq_lens_l
    ...
    cum[1:] = gls.cumsum(0)
    req_repeats = torch.repeat_interleave(torch.arange(num_reqs, device=device), gls)
    tok_idx_in_req = torch.arange(N_total, device=device) - cum[:-1].repeat_interleave(
        gls
    )
    ...
    scales_bf = torch.pow(2.0, scale_bytes.to(torch.float32) - 127.0).to(torch.bfloat16)
    ...
    out[req_repeats, out_token_idx, TOKEN_FP8_DIM : TOKEN_FP8_DIM + TOKEN_BF16_DIM] = (
        bf16_vals
    )
...
def dequantize_and_gather_k_cache(
    ...
    try:
        ...
        if _use_sm86_reference():
            torch.ops.vllm.deepseek_v4_dequant_gather_k_cache_sm86(
                out,
                k_cache,
                seq_lens,
                gather_lens,
