...
def _fused_inv_rope_fp8_quant_pyref(
    ...
    # Match the layouts consumed by fp8_einsum: FP8 is group-major with a
    # contiguous [T, D] slice per group, while scales are [G, S, T]-backed so
    # the token dimension has stride 1. SM86 never consumes packed TMA scales.
    del tma_aligned_scales
    out_fp8 = fp8_vals.transpose(0, 1).contiguous().transpose(0, 1)
    out_scale = scales.permute(1, 2, 0).contiguous().permute(2, 0, 1)
...
def _fused_inv_rope_fp8_quant_sm86_triton(
    ...
    # Keep the token dimension contiguous, matching the MN-major scale layout
    # expected by fp8_einsum without requiring TMA padding on SM86.
    scale_buf = torch.empty(
        ...
        (n_groups, num_tokens, num_scale_blocks),
        (num_scale_blocks * num_tokens, 1, num_tokens),
    )
    ...
    out_fp8 = fp32_buf.to(fp8_dtype).transpose(0, 1)
...
try:
    ...
    def _fused_inv_rope_fp8_quant_sm86_op_fake(
        ...
        return out_fp8, out_scale.transpose(0, 1)
