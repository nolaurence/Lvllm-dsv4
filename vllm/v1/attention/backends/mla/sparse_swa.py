...
class _SWAWarmupCompat:
    """Compatibility shim for V4.1's newer kernel warmup API.

    Runtime metadata generation in this branch already uses the Triton
    launchers above.  V4.1 only needs these objects during optional JIT
    warmup, so a no-op registration keeps the model importable without
    changing the existing execution path.
    """

    def register_warmup(self, **kwargs) -> None:
        del kwargs


_COMPUTE_SWA_INDICES_AND_LENS_KERNEL = _SWAWarmupCompat()
_COMPUTE_DSPARK_NONCAUSAL_SWA_INDICES_KERNEL = _SWAWarmupCompat()
