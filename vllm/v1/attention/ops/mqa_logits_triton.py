...
def _fp8_paged_mqa_logits_kernel(
    ...
    # `context_lens` is flattened to one entry per Q row: [B*next_n], so
    # 2D (B, next_n) MTP layouts keep their per-row lengths.
...
def fp8_paged_mqa_logits_triton(
    ...
    Args:
        ...
        context_lens:  [B] or [B, next_n] int32; flattened to one entry per
            Q row (matches DeepGEMM's 2D MTP semantics)
    ...
    if context_lens.dim() == 2:
        ...
        context_lens = context_lens.repeat_interleave(next_n)
    assert context_lens.numel() == B * next_n
