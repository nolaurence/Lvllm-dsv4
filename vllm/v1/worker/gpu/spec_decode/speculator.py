...
class DraftModelSpeculator(BaseSpeculator):
    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        ...
        # Widen buffers only when the target feeds the drafter an
        # HC-multiplexed residual. GLM-5.3-Flash carries ``hc_mult`` in its
        # config but its MTP head consumes the regular hidden state instead.
