...
class GPUModelRunner(
    ...
    def sample_tokens(
        ...
        if decode_stage_profile is not None:
            self._lvllm_decode_stage_profile_pending = None
            decode_stage_profile["sample_start"] = torch.cuda.Event(enable_timing=True)
            decode_stage_profile["sample_end"] = torch.cuda.Event(enable_timing=True)
        ...
        if decode_stage_profile is not None:
            ...
            self._lvllm_decode_stage_profile_calls = (
                getattr(self, "_lvllm_decode_stage_profile_calls", 0) + 1
            )
            ...
            self._lvllm_decode_stage_profile_wall_ms = (
                getattr(self, "_lvllm_decode_stage_profile_wall_ms", 0.0)
                + (time.perf_counter() - decode_stage_profile["wall_start"]) * 1000
            )
            calls = self._lvllm_decode_stage_profile_calls
            interval = int(
                os.getenv("LVLLM_PROFILE_DECODE_STAGES_INTERVAL", "8") or "8"
            )
