...
class Worker(WorkerBase):
    ...
    def _configure_lk_threads_for_worker(self) -> None:
        ...
        per_worker_threads = _parse_positive_int_env("LVLLM_LK_THREADS_PER_WORKER")
        ...
        if per_worker_threads is not None:
            ...
            worker_cpu_offset = (per_worker_threads + 1) * worker_local_rank
        ...
        if callback_cpu is None:
            callback_cpu_base = _parse_positive_int_env("LVLLM_LK_CALLBACK_CPU_BASE")
        ...
        if callback_cpu not in available_cpus:
            callback_cpu = available_cpus[
                min(len(available_cpus) - 1, worker_cpu_offset + worker_threads - 1)
            ]
